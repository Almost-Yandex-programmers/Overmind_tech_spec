...
# 1. Цель проекта
Цель проекта - разработать систему онлайн рабочее пространство, позволяющее наладить рабочие процессы, связанные с организацией людей, удобно хранить и взаимодействовать с информацией (записки, todo-планы, текстовые материалы в формате PDF). 
# 2. Описание системы
...
Приложение предусматривает два вида функционала. Есть отдельный функционал для команды, а также отдельный функционал для обычного пользователя.
...
- возможность создания, редактирования и удаления страниц, записок, to-do листов и ярлыков
- экспорт страниц в форматы .pdf, .html, .md, telegraph.
### 2.4.2. Функционал обычного пользователя

Возможности обычного Пользователя будут сильно отличаться от возможностей команды. Доступного функционала будет достаточно для того, чтобы пользоваться приложением в одиночку. 

- редактирование данных профиля
- обычный Пользователь сможет создавать: страницы записки, to-do листы и ярлыки, а также редактировать и удалять их 
- экспорт страниц в форматы .pdf, .html, .md, telegraph.
...
> Обработка текста в markdown будет происходить в момент написания текста, создание блока текста, например заголовка первого порядка, будет писать в кэше "# с пустым заголовком, пустой заголовок будет обрабатываться и не будет показывать пользователю", после каждого нажатия на "Enter" будет происходить сохранение.

Каждое нажатие "Enter" будет делать в кэше два переноса строки.
## 2.6. Функционал создания записок
Обычная текстовая записка. Помимо текста здесь будет храниться дата и время последнего редактирования.
